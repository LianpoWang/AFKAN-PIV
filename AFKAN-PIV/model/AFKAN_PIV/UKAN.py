...
class D_SingleConv(nn.Module):#（归一化 → 激活 → 卷积） + （激活 + 全连接）
    def __init__(self, in_ch, h_ch):
...
class UKan_Hybrid(nn.Module):
    def __init__(self, T, ch, ch_mult, attn, num_res_blocks, dropout):
        ...
        attn = []
        self.head = nn.Conv2d(384, ch, kernel_size=3, stride=1, padding=1)
        ...
        for i, mult in enumerate(ch_mult):
            ...
            for _ in range(num_res_blocks):
                ...
                chs.append(now_ch)
            if i != len(ch_mult) - 1:
                self.downblocks.append(DownSample(now_ch))#用步长为2的卷积实现下采样
                chs.append(now_ch)
        ...
        for i, mult in reversed(list(enumerate(ch_mult))):
            ...
            for _ in range(num_res_blocks + 1):
                ...
                now_ch = h_ch
            if i != 0:
                self.upblocks.append(UpSample(now_ch))#步长为一卷积后用上采样函数上采样
        assert len(chs) == 0

        self.tail = nn.Sequential(
            nn.GroupNorm(32, now_ch),
            Swish(),
            nn.Conv2d(now_ch, 128, 3, stride=1, padding=1)
        )

        #
        embed_dims = [384, 1024, 1280]
    ...
    def forward(self, x, t):
        ...
        B = x.shape[0]
        h, H, W = self.patch_embed3(h)#卷积1

        for i, blk in enumerate(self.kan_block1):#全连接 → 归一化 → kan → 正则化（随意丢弃路径） → 从头残差连接
            h = blk(h, H, W, temb)
        ...
        h, H, W= self.patch_embed4(h)#卷积2
        for i, blk in enumerate(self.kan_block2):#全连接 → 归一化 → kan → 正则化（随意丢弃路径） → 从头残差连接
            h = blk(h, H, W, temb)
        ...
        ### Stage 4
        # 激活（（归一化 → 激活 → 卷积） + （激活 + 全连接）后上采样）
        h = swish(F.interpolate(self.decoder1(h, temb), scale_factor=(2, 2), mode='bilinear'))

        h = torch.add(h, t4)#跳跃连接

        _, _, H, W = h.shape
        h = h.flatten(2).transpose(1, 2)
        for i, blk in enumerate(self.kan_dblock1):#全连接 → 归一化 → kan → 正则化（随意丢弃路径） → 从头残差连接
            h = blk(h, H, W, temb)

        ### Stage 3
        h = self.dnorm3(h)#归一化3
        h = h.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # 激活（（归一化 → 激活 → 卷积） + （激活 + 全连接）后上采样）
        h = swish(F.interpolate(self.decoder2(h, temb), scale_factor=(2, 2), mode='bilinear'))

        h = torch.add(h, t3)#跳跃连接

        # Upsampling
        for layer in self.upblocks:
            if isinstance(layer, ResBlock):
                #print(h.shape,hs.pop().shape)
                h = torch.cat([h, hs.pop()], dim=1)#和前半部分残差连接
            h = layer(h, temb)
        h = self.tail(h)
